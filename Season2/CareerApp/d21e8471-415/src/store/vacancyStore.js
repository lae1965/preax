...
export const useVacancyStore = create((set, get) => ({
  ...
  fetchVacancyList: async (city = '', page = 1, today = false) => {
    try {
      set({ loading: true });
      const result = await fetchAPI(`https://api.hh.ru/vacancies/?text=${defQuery}${city}&only_with_salary=${defOnlyWithSalary}&page=${page}&per_page=${defPerPage}&order_by=${defSort}`);
      const res = schemeResultVacancy(result.items, get().blackList);
